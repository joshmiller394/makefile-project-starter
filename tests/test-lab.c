...
{
    // Test adding a null pointer as data
    list_add(lst_, NULL);
    TEST_ASSERT_TRUE(lst_->size == 1);
    TEST_ASSERT_TRUE(lst_->head->next->data == NULL);
    TEST_ASSERT_TRUE(lst_->head->prev->data == NULL);
}
...
void test_multipleAddsAndRemovals(void)
{
    // Add and remove multiple elements to check overall consistency
    for (int i = 0; i < 10; i++)
    {
        list_add(lst_, alloc_data(i));
    }
...
{
    populate_list(); // makes 4->3->2->1->0
    int *rval = list_remove_index(lst_, lst_->size - 1);
    TEST_ASSERT_TRUE(lst_->size == 4);
    TEST_ASSERT_TRUE(*rval == 0);  // was "== 4", should be "== 0"
    free(rval);
    ...
    // optionally check backwards, etc.
}
...
int main(void) {
    ...
    RUN_TEST(test_multipleAddsAndRemovals);
